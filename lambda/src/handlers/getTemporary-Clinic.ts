...
export const handler = async (
  ...
  try {
    ...
    const formattedJobs = temporaryJobs.map((job: any) => ({
      ...
      rate: job.rate ?? (job.pay_type === "per_transaction" ? job.rate_per_transaction : job.pay_type === "percentage_of_revenue" ? job.revenue_percentage : job.hourly_rate) ?? 0,
      payType: job.pay_type || "per_hour",
