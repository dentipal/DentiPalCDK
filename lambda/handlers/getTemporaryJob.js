...
const handler = async (event) => {
    try {
        ...
        try {
            ...
            return {
                ...
                headers: {
                    ...
                    "Access-Control-Allow-Headers": "Content-Type, Authorization", // Allow headers for CORS
                }
            };
        }
        ...
        // Fetch the clinic address details (city, state, pincode)
        const clinicAddress = {
            city: job.city?.S || '',
            state: job.state?.S || '',
        ...
        // Format the temporary job response
        const temporaryJob = {
            jobId: job.jobId?.S || '',
            ...
            fullAddress: job.fullAddress?.S || `${job.addressLine1?.S || ''} ${job.addressLine2?.S || ''} ${job.addressLine3?.S || ''}`,
