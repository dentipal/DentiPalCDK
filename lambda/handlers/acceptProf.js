...
// ADD: EventBridge for chat system message trigger
const {
  EventBridgeClient,
  PutEventsCommand
...
function getCognitoGroups(event) {
  // REST API (Lambda proxy) -> event.requestContext.authorizer.claims['cognito:groups']
  const claimsV1 = event?.requestContext?.authorizer?.claims;
  let groups =
  ...
  if (!groups) {
    const claimsV2 = event?.requestContext?.authorizer?.jwt?.claims;
...
function getClinicIdFromEvent(event) {
  // REST API v1
  const claimsV1 = event?.requestContext?.authorizer?.claims;
...
exports.handler = async (event) => {
  try {
    ...
    const clinicUserSub = await validateToken(event);
    ...
    const isAllowed = groups.some((g) => ALLOWED.has(g));
    ...
    const applicationId = matchingItem.applicationId?.S;
    if (!applicationId) {
      return json(500, {
